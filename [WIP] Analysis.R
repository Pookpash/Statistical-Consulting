...
pn2pw <- function(mu1,mu2,mu3,sigma1,sigma2,sigma3,kappa,alpha,beta,gamma,N){
  for(i in 1:3){
  ...
  parvect <- c(tmu1,tmu2,tmu3,tsigma1,tsigma2,tsigma3,tkappa,talpha,tbeta,tgamma)
  return(parvect)
}
...
pw2pn <- function(parvect,N){
  ...
  kappa <- exp(parvect[(3*2*N+1):(7*N)])
  alpha <- exp(parvect[(7*N+1):(8*N)])
  beta <- exp(parvect[(8*N+1):(9*N)])
  gamma <- diag(N)
  if(N>1){
    gamma[!gamma] <- exp(parvect[(9*N+1):(length(parvect))])
    gamma <- gamma/apply(gamma,1,sum)           
  }
  delta <- solve(t(diag(N)-gamma+1),rep(1,N))
  return(list(mu1=mu1,mu2=mu2,mu3=mu3,sigma1=sigma1,sigma2=sigma2,sigma3=sigma3,
              kappa=kappa,alpha=alpha,beta=beta,gamma=gamma,delta=delta))
}
...
mle <- function(obs,mu01,mu02,mu03,sigma01,sigma02,sigma03,kappa0,alpha0,beta0,gamma0,N){
  parvect <- pn2pw(mu01,mu02,mu03,sigma01,sigma02,sigma03,kappa0,alpha0,beta0,gamma0,N)
  ...
  return(list(mu1=pn$mu1,mu2=pn$mu2,mu3=pn$mu3,sigma1=pn$sigma1,sigma2=pn$sigma2,sigma3=pn$sigma3,
              kappa=pn$kappa,alpha=pn$alpha,beta=pn$beta,gamma=pn$gamma,delta=pn$delta,mllk=mod$minimum))
}
