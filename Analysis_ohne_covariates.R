...
data <- read.csv("subsample_50.csv")
...
fitmult <- function(obs,n_fits,N){
        ...
        for (i in 1:n_fits){
                ...
                modl[[i]] <- mle(obs,c(runif(N,3,35)),c(runif(N,80,175)),c(runif(N,30,55)),c(runif(N,10,150)),c(runif(N,2,10)),
                                 c(runif(N,25,100)),c(runif(N,10,30)),c(runif(N,10,80)),c(runif(N,-2,2)),c(runif(N,2,7)),
                                 mat,N)
        }
        return(modl)
}
